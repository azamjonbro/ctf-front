<template>
  <div class="min-h-screen bg-[#0B1020] flex items-center justify-center px-4 relative overflow-hidden">
    <!-- Matrix rain canvas -->
    <canvas ref="matrixCanvas" class="absolute inset-0 transition-opacity duration-1000 pointer-events-none z-0" :class="isRegistering ? 'opacity-[0.25]' : 'opacity-[0.08]'"></canvas>
    ...
    <div v-if="isRegistering" class="w-full max-w-md p-6 rounded-lg border border-cyber-primary/20 bg-[#0B1020]/90 glass-panel font-mono text-xs space-y-6 shadow-[0_0_50px_rgba(0,255,136,0.15)] relative z-10 overflow-hidden">
      ...
      <div class="flex items-center justify-between border-b border-cyber-primary/20 pb-3 text-cyber-primary">
        <div class="flex items-center space-x-2">
          ...
          <span class="font-bold tracking-widest text-shadow-neon">// ALGORITM.CTF SECURE SIGNUP</span>
        </div>
      ...
      <div class="space-y-2 text-slate-300 min-h-[100px] flex flex-col justify-end">
        ...
        <div v-if="loadingProgress > 25" class="flex items-center space-x-2">
          <span class="text-cyber-primary">></span>
          <span>SUBMITTING SECURE SCHEMA ENTRY... <span class="text-cyber-primary font-bold">OK</span></span>
        </div>
        <div v-if="loadingProgress > 50" class="flex items-center space-x-2">
          <span class="text-cyber-primary">></span>
          <span>GENERATING HASH PASSPHRASE... <span class="text-cyber-primary font-bold">SUCCESS</span></span>
        </div>
        <div v-if="loadingProgress > 75" class="flex items-center space-x-2">
          <span class="text-cyber-primary">></span>
          <span>ALLOCATING WORKSPACE SEGMENTS... <span class="text-cyber-secondary font-bold">DONE</span></span>
        </div>
      </div>
    ...
    <div v-else class="w-full max-w-md p-8 rounded-lg glass-panel border border-white/10 relative z-10 space-y-6">
      ...
      <form @submit.prevent="handleRegister" class="space-y-4">
        <div class="space-y-1">
          <label class="text-xs uppercase font-mono tracking-wider text-slate-400 block" for="username">Foydalanuvchi nomi</label>
          <input
            id="username"
          ...
          />
          <span v-if="v$.username.$error" class="text-[10px] text-cyber-danger font-mono block">Majburiy maydon (harf/raqam, kamida 3 ta belgi)</span>
        </div>

        <div class="space-y-1">
          <label class="text-xs uppercase font-mono tracking-wider text-slate-400 block" for="email">Elektron pochta manzili</label>
          <input
            id="email"
            v-model="state.email"
            type="email"
          ...
          />
          <span v-if="v$.email.$error" class="text-[10px] text-cyber-danger font-mono block">To'g'ri elektron pochta manzilini kiriting</span>
        </div>

        <div class="space-y-1">
          <label class="text-xs uppercase font-mono tracking-wider text-slate-400 block" for="password">Parol yarating</label>
          <input
            id="password"
      ...
      <div class="text-center text-xs text-slate-500 font-mono">
        ...
        <router-link to="/login" class="text-cyber-primary hover:underline">// TIZIMGA KIRISH</router-link>
      </div>
    </div>
  </div>
</template>
...
const handleRegister = async () => {
  ...
  try {
    ...
    setTimeout(() => {
      ...
      toast.success('Ro\'yxatdan o\'tish yakunlandi! Endi tizimga kirishingiz mumkin.');
      router.push('/login');
    }, 1200);
  } catch (error) {
    clearInterval(progressInterval);
