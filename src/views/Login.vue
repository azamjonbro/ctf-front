<template>
  <div class="min-h-screen bg-[#0B1020] flex items-center justify-center px-4 relative overflow-hidden">
    <!-- Matrix rain canvas -->
    <canvas ref="matrixCanvas" class="absolute inset-0 transition-opacity duration-1000 pointer-events-none z-0" :class="isLoggingIn ? 'opacity-[0.25]' : 'opacity-[0.08]'"></canvas>

    <!-- Cyber Background Accent Glows -->
    <div class="absolute top-1/4 left-1/4 h-[350px] w-[350px] bg-cyber-primary/10 rounded-full blur-[120px] pointer-events-none z-0"></div>
    <div class="absolute bottom-1/4 right-1/4 h-[350px] w-[350px] bg-cyber-accent/15 rounded-full blur-[120px] pointer-events-none z-0"></div>

    <!-- Hacker Loading State -->
    <div v-if="isLoggingIn" class="w-full max-w-md p-6 rounded-lg border border-cyber-primary/20 bg-[#0B1020]/90 glass-panel font-mono text-xs space-y-6 shadow-[0_0_50px_rgba(0,255,136,0.15)] relative z-10 overflow-hidden">
      ...
      <div class="flex items-center justify-between border-b border-cyber-primary/20 pb-3 text-cyber-primary">
        <div class="flex items-center space-x-2">
          ...
          <span class="font-bold tracking-widest text-shadow-neon">// ALGORITM.CTF SECURE LOGIN</span>
        </div>
      ...
      <div class="space-y-2 text-slate-300 min-h-[100px] flex flex-col justify-end">
        ...
        <div v-if="loadingProgress > 25" class="flex items-center space-x-2">
          <span class="text-cyber-primary">></span>
          <span>NEGOTIATING HANDSHAKE PROTOCOLS... <span class="text-cyber-primary font-bold">OK</span></span>
        </div>
        <div v-if="loadingProgress > 50" class="flex items-center space-x-2">
          <span class="text-cyber-primary">></span>
          <span>AUTHENTICATING OPERATOR CREDENTIALS... <span class="text-cyber-primary font-bold">VERIFIED</span></span>
        </div>
        <div v-if="loadingProgress > 75" class="flex items-center space-x-2">
          <span class="text-cyber-primary">></span>
          <span>DOWNLOADING USER SESSION STATES... <span class="text-cyber-secondary font-bold">DONE</span></span>
        </div>
      </div>
    ...
    <div v-else class="w-full max-w-md p-8 rounded-lg glass-panel border border-white/10 relative z-10 space-y-6">
      <div class="text-center space-y-2">
        <h1 class="text-3xl font-extrabold font-mono tracking-wider text-cyber-primary">
          // ALGORITM.<span class="text-cyber-secondary">CTF</span>
        </h1>
        <p class="text-xs text-slate-400">XAVFSIZLIK KLIENT TUGUNINI O'RNATISH UCHUN SHAXSNI TASDIQLANG</p>
      </div>

      <form @submit.prevent="handleLogin" class="space-y-4">
        <div class="space-y-1">
          <label class="text-xs uppercase font-mono tracking-wider text-slate-400 block" for="usernameOrEmail">Foydalanuvchi nomi yoki elektron pochta</label>
          <input
            id="usernameOrEmail"
          ...
          />
          <span v-if="v$.usernameOrEmail.$error" class="text-[10px] text-cyber-danger font-mono block">Maydonni to'ldirish shart</span>
        </div>

        <div class="space-y-1">
          <label class="text-xs uppercase font-mono tracking-wider text-slate-400 block" for="password">Xavfsizlik paroli</label>
          <input
            id="password"
          ...
          />
          <span v-if="v$.password.$error" class="text-[10px] text-cyber-danger font-mono block">Minimal uzunlik - 6 ta belgi</span>
        </div>

        <!-- Mathematical Captcha -->
        <div class="space-y-1">
          <label class="text-xs uppercase font-mono tracking-wider text-slate-400 block">Matematik captcha: <span class="text-cyber-primary font-bold">{{ captchaQuestion }}</span></label>
      ...
      <div class="text-center text-xs text-slate-500 font-mono">
        ...
        <router-link to="/register" class="text-cyber-secondary hover:underline">// RO'YXATDAN O'TISH</router-link>
      </div>
    </div>
  </div>
</template>
...
const handleLogin = async () => {
  ...
  try {
    ...
    state.captchaAnswer = '';
    loadCaptcha();
    const errorMsg = error?.error?.message || 'Avtorizatsiya ma\'lumotlari noto\'g\'ri';
